...
int main()
{
  ...
  struct SClient
  {
    int sockid = 0;
    int epid = 0;
    SClient(int s, int e) : sockid(s),epid(e){}
  };
    ...
    for (int i = 0; i < nfds; ++i)
    {
      ...
      if (events[i].data.fd == sockid)
      {
        ...
        else
        {
          ...
          clients.push_back(SClient(clientsock, clientepid));
          usleep(1000);
        }
      }
    }
    /*if (clientepid)
    {
      ...
      for (int i = 0; i < nfds; ++i)
      {
        ...
        if (events[i].events & EPOLLIN)
        {
          ...
          int ret = ::recv(clientsock, buffer, MAX_BUFSIZE, 0);
          ...
          std::cout << "接收字符串内容为: " << buffer << std::endl;
        }
      }
    }
    */
    ...
    for (auto&s : clients)
    {
      cliendid ++;
      bzero(events, MAX_SERVER_EVENT * sizeof(epoll_event));
      nfds = epoll_wait(s.epid, events, MAX_SERVER_EVENT, 20);
      for (int i=0;i < nfds; ++i)
      {
        if (events[i].events & EPOLLIN)
        {
          ...
          int ret = ::recv(s.sockid, buffer, MAX_BUFSIZE, 0);
          std::cout << "socket recv data size: " << ret << std::endl;
          if (ret > 0 && ret < MAX_BUFSIZE)
          {
            std::cout << "data: ";
          ...
          std::cout << "client:" << cliendid << " 接收字符串内容为: " << buffer << std::endl << std::endl;

          // 转发
          char sendbuff[MAX_BUFSIZE+20];
          bzero(sendbuff, sizeof(sendbuff));
          sprintf(sendbuff,"client id: %d, value: %s",cliendid, buffer);
          ...
          for (auto &c : clients)
          {
            ...
            if (tt==cliendid)
              continue;
            ret = ::send(c.sockid, sendbuff, strlen(sendbuff), 0);
            std::cout << "转发client: " << tt << " ret: " << ret << std::endl << std::endl;
          }
        }
      }
    }
